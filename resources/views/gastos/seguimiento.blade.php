<x-app-layout>
    ...
    <div class="flex flex-col w-full text-gray-700 px-2 md:px-8">
        ...
        <div class="w-full text-lg font-semibold">Seguimiento Gastos</div>
        ...
        <div class="w-full rounded-b-lg bg-white p-3 flex flex-col"> <!--CONTENIDO-->
            <div class="w-full flex flex-row">
                <div class="w-1/3">
                    <form action="{{route('seguimiento_gastos')}}" class="">
            ...
            <div class="w-full flex justify-center pt-5 flex-col"> <!--TABLA DE CONTENIDO-->
                ...
                <table>
                    <tr class="">
                        ...
                        <td class="border border-gray-300 font-semibold bg-slate-600 text-gray-200 p-1 text-sm"><center>Dia</td>
                ...
                ?>
                    <tr class="">
                        ...
                        <td class="border border-gray-300 font-light {{$color?'bg-lime-100':''}} text-gray-700 p-1 text-xs">{{$registro->dia_gasto}}</td>
                        <td class="border border-gray-300 font-light {{$color?'bg-lime-100':''}} text-gray-700 p-1 text-xs">{{$registro->descripcion}}</td>
                        <td class="border border-gray-300 font-light {{$color?'bg-lime-100':''}} text-gray-700 p-1 text-xs"><center>${{number_format($registro->gasto,2)}}</td>
                    </tr>
                <?php
                    $color=!$color;
                    }
        ...
        <script>
            ...
            function borrar_gasto(id,dia,tipo)
            {
                if(confirm("Ha indicado borrar el gasto "+tipo+" del dia "+dia+"\n\n¿Desea continuar?"))
                {
                    var xmlhttp = new XMLHttpRequest();
                    xmlhttp.open("POST", "/gasto_borrar", true);
